...
def main(argv=None, working_dir=None):
    ...
    # 切换工作目录：默认仍按原逻辑切到脚本所在目录；允许外部传入 working_dir
    ...
    if options.do_statis:
        target = args[0] if len(args) > 0 else (argv[2] if len(argv) > 2 else '')
        if target:
            static_andorid_log(ylog_parser_relative_path + target)
        return

    if options.do_versioninfo:
        target = args[0] if len(args) > 0 else (argv[2] if len(argv) > 2 else '')
        if target:
            get_version_info(ylog_parser_relative_path + target)
        return
